...
int update_time_display(time_display *time_info){
    if(is_updating_rtc()!=0){
        printf("Values are being updated, so it can't be acessed.");
        return 1;
    }
    uint8_t output;

    if(rtc_output(SECONDS,&output)!=0){
        printf("Failed to acess SECONDS register.");
        return 1;
    }
    ...
    if(rtc_output(MINUTES,&output)!=0){
        printf("Failed to acess MINUTES register.");
        return 1;
    }
    convert_binary(&output);
    ...
    if(rtc_output(HOURS,&output)!=0){
        printf("Failed to acess HOURS register.");
        return 1;
    }
    convert_binary(&output);
